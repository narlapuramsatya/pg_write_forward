...
SELECT * FROM pg_write_forward_status();
                                 primary_conninfo                                  | consistency | enabled | connected | forwarded_count | last_remote_lsn | forwarded_failures | cancellations | reconnects 
-----------------------------------------------------------------------------------+-------------+---------+-----------+-----------------+-----------------+--------------------+---------------+------------
 host=/home/azureuser/pg/pwf_demo/sockets port=55501 dbname=postgres user=postgres | session     | t       | f         |               0 |                 |                  0 |             0 |          0
(1 row)
...
INSERT INTO pwf_demo (payload) VALUES (&#x27;explained&#x27;);
                                                QUERY PLAN                                                
----------------------------------------------------------------------------------------------------------
 Insert on public.pwf_demo  (cost=0.00..0.01 rows=0 width=0) (actual time=0.010..0.010 rows=0.00 loops=1)
...
SELECT * FROM pg_write_forward_status();
                                 primary_conninfo                                  | consistency | enabled | connected | forwarded_count | last_remote_lsn | forwarded_failures | cancellations | reconnects 
-----------------------------------------------------------------------------------+-------------+---------+-----------+-----------------+-----------------+--------------------+---------------+------------
 host=/home/azureuser/pg/pwf_demo/sockets port=55501 dbname=postgres user=postgres | global      | t       | t         |               1 | 0/032695A8      |                  0 |             0 |          0
(1 row)
...
SELECT * FROM pg_write_forward_status();
                                 primary_conninfo                                  | consistency | enabled | connected | forwarded_count | last_remote_lsn | forwarded_failures | cancellations | reconnects 
-----------------------------------------------------------------------------------+-------------+---------+-----------+-----------------+-----------------+--------------------+---------------+------------
 host=/home/azureuser/pg/pwf_demo/sockets port=55501 dbname=postgres user=postgres | global      | t       | f         |               1 | 0/032695A8      |                  0 |             0 |          0
(1 row)
...
SELECT * FROM pg_write_forward_status();
                                 primary_conninfo                                  | consistency | enabled | connected | forwarded_count | last_remote_lsn | forwarded_failures | cancellations | reconnects 
-----------------------------------------------------------------------------------+-------------+---------+-----------+-----------------+-----------------+--------------------+---------------+------------
 host=/home/azureuser/pg/pwf_demo/sockets port=55501 dbname=postgres user=postgres | global      | t       | t         |               2 | 0/03269660      |                  0 |             0 |          0
(1 row)
